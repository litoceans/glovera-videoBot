...
class OnnxWrapper {
    ...
    constructor(path: string, force_onnx_cpu: boolean = true) {
        ...
        this.resetStates();  // Initial reset of the state
    ...
    async call(x: number[][], sr: number): Promise<number[][]> {
        ...
        // Modify reset logic to avoid redundant resets
        if (this._last_batch_size === 0 || this._last_sr !== sr || this._last_batch_size !== batch_size) {
            console.log("Reset state triggered due to state change", {
                batch_size,
                _last_sr: this._last_sr,
                _last_batch_size: this._last_batch_size
            });
        ...
        if (sr === 8000 || sr === 16000) {
            ...
            this._last_sr = sr;  // Update the sample rate
            this._last_batch_size = batch_size;  // Update the batch size
